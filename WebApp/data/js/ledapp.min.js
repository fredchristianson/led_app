'use strict';const API_BASE_DINING_ROOM$$module$="http://192.168.10.133/api/",API_BASE_LIVING_ROOM$$module$="http://192.168.10.120/api/",API_BASE_SAME_HOST$$module$="/api/",API_BASE$$module$=API_BASE_SAME_HOST$$module$;
class Api$$module${constructor(){this.baseUrl=API_BASE_SAME_HOST$$module$}async get(a){return new Promise((b,c)=>{const d=new XMLHttpRequest;d.onload=e=>{b(d.responseText)};d.onerror=e=>{c(d.response)};d.open("GET",this.baseUrl+a);d.send()})}async getJSON(a,b){a=await this.get(a,b);return JSON.parse(a)}async post(a,b){return new Promise((c,d)=>{const e=new XMLHttpRequest;e.open("POST",this.baseUrl+a,!0);e.onreadystatechange=()=>{e.readyState==XMLHttpRequest.DONE&&c(e.resonseText)};e.onerror=f=>{d(e.response)};
e.setRequestHeader("Content-Type","application/json");e.send(b);return!0})}}const LOG_LEVEL$$module$={Dom:{debug:!1},LedSelector:{debug:!1},ColorSlider:{debug:!0}};
...
!1;this.changed()}enableSaturation(a=!0){this.saturationEnabled=a;this.changed()}disableLevel(){this.levelEnabled=!1;this.changed()}enableLevel(a=!0){this.levelEnabled=a;this.changed()}setSelectedRange(a,b){this.first.setLedPercent(a);this.last.setLedPercent(b);this.changed()}hueRangeValueToHtmlHue(a){if(this.hueMap[a])return this.hueMap[a];var b=DOM$$module$.getPixel(this.image,this.image.naturalWidth*a/255,50);b=rgb2hsv$$module$(b[0],b[1],b[2]).hue;return this.hueMap[a]=b}getStartHslColor(){return this.first}getEndHslColor(){return this.gradientEnabled?
this.last:this.first}updateColors(a){const b=this.first;var c=this.gradientEnabled?this.last:this.first;const d=new HueGradient$$module$(b.getHueRangeValue(),c.getHueRangeValue()),e=new LevelGradient$$module$(b.getLevel(),c.getLevel());c=new SaturationGradient$$module$(b.getSaturation(),c.getSaturation());this.hueEnabled&&d.apply(a,b.getLedPercent(),this.last.getLedPercent());this.levelEnabled&&e.apply(a,b.getLedPercent(),this.last.getLedPercent());this.saturationEnabled&&c.apply(a,b.getLedPercent(),
this.last.getLedPercent());a.forEach(f=>{f.htmlColor=null!=f.hue?`hsl(${f.hue},${null===f.saturation?100:f.saturation}%,${null===f.level?50:f.level}%)`:"hsl(0,0%,0%)"})}}
...
b){this.logger.debug("start touch "+a.moveTarget.id)}touchMove(a,b){try{this.logger.debug(`move touch ${a.moveTarget.id} ${a.moveTarget.className} ${a.currentPos.x},${a.lastMovement.dx}`);var c=a.moveTarget,d=a.currentPos.x;if(!isNaN(d)){for(;null!=c&&c!=this.container;){d+=c.offsetLeft;if(isNaN(d))return;c=c.parentNode}d=Math.max(d,0);d=Math.min(d,this.image.naturalWidth-1);this.moveable.style.left=`${d}px`;this.hueRangeOffset=d;this.hueRangeValue=Math.round(255*d/this.image.naturalWidth);this.hslColor.setHueRangeValue(this.hueRangeValue);
this.logger.debug(`hue: ${d}, ${this.image.naturalWidth}, ${this.hueRangeValue}`)}}catch(e){this.logger.error("move failed ",e)}}touchEnd(a,b){this.logger.debug("end touch "+a.moveTarget.id)}onColorChanged(a){this.element.style.backgroundColor=this.hslColor.getHsl()}}
...
class LedApp$$module${constructor(){this.logger=new Logger$$module$("LedApp");this.logger.debug("LedApp running");this.ledSelector=new LedSelector$$module$(this);this.hueSelector=new HueSelector$$module$(this);this.xhrOpen=!1;DOM$$module$.onClick(".run-commands",this.runCommands.bind(this));this.zoom=1;this.animateSpeedPerSecond=0;DOM$$module$.onClick("#gradient",this.toggleGradient.bind(this));DOM$$module$.onClick("#hue-off",this.toggleHue.bind(this));DOM$$module$.onClick("#level-off",this.toggleLevel.bind(this));
DOM$$module$.onClick("#saturation-off",this.toggleSaturation.bind(this));this.saturationFirst=new SaturationSelector$$module$("#saturation-first",colorMgr$$module$.first);this.saturationLast=new SaturationSelector$$module$("#saturation-last",colorMgr$$module$.last);this.levelFirst=new LevelSelector$$module$("#level-first",colorMgr$$module$.first);this.levelLast=new LevelSelector$$module$("#level-last",colorMgr$$module$.last);colorMgr$$module$.observe(this.onColorChanged.bind(this));DOM$$module$.onClick(".create-command.hue",
this.createHueCommand.bind(this));DOM$$module$.onClick(".create-command.level",this.createLevelCommand.bind(this));DOM$$module$.onClick(".create-command.saturation",this.createSaturationCommand.bind(this));DOM$$module$.onClick(".create-command.add-hsl",this.createHSLCommands.bind(this));this.initialize()}onColorChanged(){this.runCommands()}isGradient(){return DOM$$module$.first("#gradient").checked}toggleHue(a,b){a.checked?(DOM$$module$.addClass(document.body,"no-hue"),colorMgr$$module$.disableHue()):
(colorMgr$$module$.enableHue(),DOM$$module$.removeClass(document.body,"no-hue"));colorMgr$$module$.changed();return!1}toggleSaturation(a,b){DOM$$module$.toggleClass(document.body,"no-saturation");colorMgr$$module$.enableSaturation(!a.checked);return!1}toggleLevel(a,b){DOM$$module$.toggleClass(document.body,"no-level",a.checked);colorMgr$$module$.enableLevel(!a.checked);return!1}toggleGradient(a,b){DOM$$module$.toggleClass(document.body,"gradient",a.checked);colorMgr$$module$.enableGradient(a.checked);
return!1}createHueCommand(){const a=colorMgr$$module$.getStartHslColor(),b=colorMgr$$module$.getEndHslColor();this.addCommand("h",this.ledSelector.getStartLedPercent(),this.ledSelector.getEndLedPercent(),a.getHueRangeValue(),b.getHueRangeValue(),this.zoom,this.animateSpeedPerSecond)}createLevelCommand(){const a=colorMgr$$module$.getStartHslColor(),b=colorMgr$$module$.getEndHslColor();this.addCommand("l",this.ledSelector.getStartLedPercent(),this.ledSelector.getEndLedPercent(),a.getLevel(),b.getLevel(),
this.zoom,this.animateSpeedPerSecond)}createSaturationCommand(){const a=colorMgr$$module$.getStartHslColor(),b=colorMgr$$module$.getEndHslColor();this.addCommand("s",this.ledSelector.getStartLedPercent(),this.ledSelector.getEndLedPercent(),a.getSaturation(),b.getSaturation(),this.zoom,this.animateSpeedPerSecond)}createHSLCommands(){colorMgr$$module$.hueEnabled&&this.createHueCommand();colorMgr$$module$.levelEnabled&&this.createLevelCommand();colorMgr$$module$.saturationEnabled&&this.createSaturationCommand()}addCommand(a,
b,c,d,e,f,h){if(null==b||null==c||isNaN(b)||isNaN(c))this.logger.error("invalid range "+b+" "+c);else{var g=DOM$$module$.getValue("#scene");DOM$$module$.setValue("#scene",g+`${a},${b},${c},${d},${e},${f},${h};\n`);this.runCommands()}}runCommands(){const a=[];for(var b=0;b<this.ledSelector.getCount();b++)a[b]={hue:null,level:null,saturation:null,htmlColor:null};DOM$$module$.getValue("#scene").split(";").forEach(c=>{c=c.trim().split(",");for(var d=1;d<c.length;d++){var e=Number.parseFloat(c[d]);isNaN(e)||
(c[d]=e)}if(1<c.length){this.logger.debug("command: "+c[0]);e=c[0];const f=c[1],h=c[2];d=null;"h"==e?d=new HueGradient$$module$(c[3],c[4]):"l"==e?d=new LevelGradient$$module$(c[3],c[4]):"s"==e&&(d=new SaturationGradient$$module$(c[3],c[4]));d?d.apply(a,f,h):this.logger.debug("unknownn command: "+e)}});colorMgr$$module$.updateColors(a);this.ledSelector.setColors(a)}updateColor(){}hueChanged(){this.htmlHue=this.hueSelector.getHtmlHue();this.hueRangeValue=this.hueSelector.getHueRangeValue();const a=
this.ledSelector.getStartLed(),b=this.ledSelector.getEndLed();this.addCommand("h",a,b,this.hueRangeValue,this.hueRangeValue,this.zoom,this.animateSpeedPerSecond);this.updateColor()}d2h(a){a=a.toString(16);return 1==a.length?"0"+a:a}toHexColor(a){return`#${this.d2h(a.r)}${this.d2h(a.g)}${this.d2h(a.b)}`}async initialize(){DOM$$module$.onClick("#save-scene",this.saveCommands.bind(this));DOM$$module$.onClick("#load-scene",this.loadCommands.bind(this));this.api=new Api$$module$;await this.readConfig()}async readConfig(){const a=
await this.api.getJSON("scene/config");a?(this.config=a,this.ledSelector.setCount(this.config.led_count),DOM$$module$.setHtml("#room-name",this.config.hostname+" - "+this.config.ip_addr),DOM$$module$.setSelectOptions("#saved-scenes",this.config.scenes,null),this.ledSelector.selectClear(),this.runCommands()):alert("Failed to get configuration")}async loadCommands(){const a=DOM$$module$.getValue("#saved-scenes");try{const b=await this.api.get(`scene/${a}`);DOM$$module$.setValue("#scene",b);DOM$$module$.setValue("#scene-name",
a);this.ledSelector.selectClear();this.runCommands()}catch(b){this.logger.error("load failed ",b)}}async saveCommands(){const a=DOM$$module$.getValue("#scene-name"),b=DOM$$module$.getValue("#scene");DOM$$module$.setValue("#saved-scenes",a);try{await this.api.post(`scene/${a}`,b)}catch(c){this.logger.error("POST failed",c),alert("save failed")}}}
function rgb2hsv$$module$(a,b,c){let d,e;let f,h,g,m,p;a/=255;b/=255;c/=255;g=Math.max(a,b,c);m=g-Math.min(a,b,c);var l=k=>(g-k)/6/m+.5;p=k=>Math.round(100*k)/100;0==m?f=h=0:(h=m/g,d=l(a),e=l(b),l=l(c),a===g?f=l-e:b===g?f=1/3+d-l:c===g&&(f=2/3+e-d),0>f?f+=1:1<f&&--f);return{hue:Math.round(360*f),saturation:p(100*h),value:p(100*g)}}
...
a,e=d.touchStart?d.touchStart.bind(d):null,f=d.touchEnd?d.touchEnd.bind(d):null,h=d.touchMove?d.touchMove.bind(d):null);this.listen("mousedown",b,g=>{function m(n){const q=n.target;k.moveTarget=q;k.lastPos=k.currentPos;k.currentPos={x:n.offsetX,y:n.offsetY};k.lastMovement={dx:k.currentPos.x-k.lastPos.x,dy:k.currentPos.y-k.lastPos.y};k.totalMovement={dx:k.currentPos.x-k.startPos.x,dy:k.startPos.y-k.lastPos.y};l.logger.debug("mousemove "+q.id);h(k,n);return!0}function p(n){const q=n.target;k.moveTarget=
q;l.logger.debug("mouseup "+q.id);document.removeEventListener("mouseup",p);document.removeEventListener("mousemove",m);f(k,n);return!0}var l=this;const k={startPos:{x:g.offsetX,y:g.offsetY},currentPos:{x:g.offsetX,y:g.offsetY},lastPos:{x:g.offsetX,y:g.offsetY},totalMovement:{dx:0,dy:0},lastMovement:{dx:0,dy:0},container:b,moveTarget:g.target},r=g.target;if(r==b||this.match(r,c))return this.logger.debug("mousedown "+r.id),e&&e(k,g),document.addEventListener("mousemove",m),document.addEventListener("mouseup",
