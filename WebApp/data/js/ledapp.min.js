...
await this.readConfig()});await this.readConfig();this.ledSelector.selectAll();this.initialized=!0}async readConfig(){const a=await this.api.getJSON("scene/config");a?(this.config=a,this.ledSelector.setCount(this.config.led_count),DOM$$module$.setSelectOptions("#saved-scenes",this.config.scenes,null),this.ledSelector.selectClear(),this.runCommands()):alert("Failed to get configuration")}async loadCommands(){const a=DOM$$module$.getValue("#saved-scenes");try{const b=await this.api.get(`scene/${a}`);
