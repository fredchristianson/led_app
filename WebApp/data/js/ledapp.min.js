'use strict';class Api$$module${constructor(){}setHostname(a){this.baseUrl=null==a?"/api/":`http://${a}/api/`}async get(a){return new Promise((b,c)=>{const d=new XMLHttpRequest;d.onload=e=>{b(d.responseText)};d.onerror=e=>{c(d.response)};d.open("GET",this.baseUrl+a);d.send()})}async getJSON(a,b){a=await this.get(a,b);return JSON.parse(a)}async post(a,b){return new Promise((c,d)=>{const e=new XMLHttpRequest;e.open("POST",this.baseUrl+a,!0);e.onreadystatechange=()=>{e.readyState==XMLHttpRequest.DONE&&
c(e.resonseText)};e.onerror=f=>{d(e.response)};e.setRequestHeader("Content-Type","application/json");e.send(b);return!0})}}const LOG_LEVEL$$module$={Dom:{debug:!1},LedSelector:{debug:!1},ColorSlider:{debug:!1}};
...
class LedApp$$module${constructor(){this.initialized=!1;this.logger=new Logger$$module$("LedApp");this.logger.debug("LedApp running");this.ledSelector=new LedSelector$$module$(this);this.hueSelector=new HueSelector$$module$(this);this.xhrOpen=!1;DOM$$module$.onClick(".run-commands",this.runCommands.bind(this));this.zoom=1;DOM$$module$.onClick("#gradient",this.toggleGradient.bind(this));DOM$$module$.onClick("#hue-off",this.toggleHue.bind(this));DOM$$module$.onClick("#level-off",this.toggleLevel.bind(this));
...
this.updateColor()}d2h(a){a=a.toString(16);return 1==a.length?"0"+a:a}toHexColor(a){return`#${this.d2h(a.r)}${this.d2h(a.g)}${this.d2h(a.b)}`}async initialize(){var a=storage$$module$.get("room");DOM$$module$.setValue("#room-select",a);DOM$$module$.onClick("#save-scene",this.saveCommands.bind(this));DOM$$module$.onClick("#load-scene",this.loadCommands.bind(this));this.api=new Api$$module$;this.api.setHostname(a);DOM$$module$.onChange("#room-select",async b=>{storage$$module$.set("room",b.value);this.api.setHostname(b.value);
await this.readConfig()});try{await this.readConfig()}catch(b){}this.ledSelector.selectAll();this.initialized=!0}async readConfig(){const a=await this.api.getJSON("scene/config");a?(this.config=a,this.ledSelector.setCount(this.config.led_count),DOM$$module$.setSelectOptions("#saved-scenes",this.config.scenes,null),this.ledSelector.selectClear(),this.runCommands()):alert("Failed to get configuration")}async loadCommands(){const a=DOM$$module$.getValue("#saved-scenes");try{const b=await this.api.get(`scene/${a}`);
DOM$$module$.setValue("#scene",b);DOM$$module$.setValue("#scene-name",a);this.ledSelector.selectClear();this.runCommands()}catch(b){this.logger.error("load failed ",b)}}async saveCommands(){if(this.initialized){var a=DOM$$module$.getValue("#scene-name"),b=DOM$$module$.getValue("#scene");DOM$$module$.setValue("#saved-scenes",a);try{await this.api.post(`scene/${a}`,b)}catch(c){this.logger.error("POST failed",c),alert("save failed")}}}}
...
b){this.find(a).forEach(c=>{c.innerHTML=b})}addOption(a,b,c=!1){this.find(a).forEach(d=>{d=document.createElement("option");d.value=b;d.innerHTML=b;c&&d.setAttribute("selected","selected");a.appendChild(d)})}setSelectOptions(a,b,c,d="--select--"){DOM$$module$.find(a).forEach(e=>{e.innerHTML="";d&&this.addOption(e,d);b.forEach(f=>{this.addOption(e,f,f==c)})})}isChecked(a){return(a=this.first(a))&&a.checked}}
class Storage$$module${constructor(){this.store=window.localStorage}get(a){return this.store.getItem(a)}set(a,b){this.store.setItem(a,b)}}const storage$$module$=new Storage$$module$,DOM$$module$=new Dom$$module$,colorMgr$$module$=new ColorManager$$module$;var $jscompDefaultExport$$module$=LedApp$$module$,module$={};module$.LedApp=LedApp$$module$;module$.default=$jscompDefaultExport$$module$;